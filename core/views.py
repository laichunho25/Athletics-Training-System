...
def _record_domains(session):
    """按「登記錄」時挑得到的範疇——三個都挑得到，課別該有的那個排前面。

    課別只是課表上的分類，不該決定當天登得了什麼：田徑課臨時補一組深蹲、
    比賽日的熱身跑，照樣要記得下來。所以三個範疇一律列出來，
    只是把這個課別最可能的那一個排在最前面（也是預設值）。
    """
    natural = [d.value for d in domains_for_session_type(session.session_type)]
    labels = dict(MetricDomain.choices)
    ordered = natural + [v for v in MetricDomain.values if v not in natural]
    return [(v, labels[v]) for v in ordered]


def _record_domain(session, raw):
    """挑中的範疇：沒挑或挑了不認得的，就用這個課別最可能的那一個。"""
    if raw in MetricDomain.values:
        return raw
    return _record_domains(session)[0][0]
...
def _session_metric_context(request, session):
    ...
    return {
        ...
        # 秒為單位的項目，「目標／完成」那兩格拆成「分」＋「秒」——
        # 800m 跑 2:05 這種成績，不用自己在心裡換算成 125
        "unit_is_time": unit in ("秒", "s", "sec"),
