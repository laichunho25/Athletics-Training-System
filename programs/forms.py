...
SECTIONS = [
    ...
    (
        _("身體狀況與緊急聯絡（KYC）"),
        _("力量訓練前必須掌握的健康資訊。如有任何不確定，請先諮詢醫生。"),
        [
            "height_cm", "weight_kg",
...
REQUIRED_CONSENTS = {
    "health_declaration": _("請確認健康申報屬實。"),
    "consent_terms": _("請閱讀並同意項目條款（包括不設退款）。"),
    "consent_data": _("請同意我們使用你的資料作訓練管理與聯絡。"),
}
...
class ApplicationForm(forms.ModelForm):
    class Meta:
        model = Application
        fields = [f for _unused, _, fields in SECTIONS for f in fields]
        widgets = {
    ...
    def __init__(self, *args, project=None, **kwargs):
        ...
        self.fields["primary_event"].empty_label = _("（未定 / 不適用）")
        self.fields["email"].help_text = _("確認信與課堂通知會寄到這裡")
        self.fields["graduation_year"].widget.attrs["placeholder"] = _("例：2028")
        self.fields["personal_best"].widget.attrs["placeholder"] = _("例：100m 11.42（2026 年 4 月）")
    ...
    def clean_birth_date(self):
        ...
        if birth_date >= today:
            raise forms.ValidationError(_("出生日期不正確。"))
    ...
    def clean(self):
        ...
        if cleaned.get("has_current_injury") and not (cleaned.get("injury_detail") or "").strip():
            self.add_error("injury_detail", _("有傷患時請描述部位與目前狀況。"))
