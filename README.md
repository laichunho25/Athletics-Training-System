...
方向與介面守則見 [docs/產品方向.md](docs/產品方向.md)，
...
| 運動員名單 | http://127.0.0.1:8200/athletes/ |
| 狀態總覽（運動員） | http://127.0.0.1:8200/dashboard/ |
| 全隊總覽（教練） | http://127.0.0.1:8200/team/ |
...
| 運動員名單 | 姓名／計劃／主項／年紀／傷患狀態一覽，可搜尋、篩選與點表頭排序，是進入個人總覽的入口 |
| 狀態總覽 | 賽事倒數、目前分期（兩者可直接在卡片上改）、ACWR 燈號、準備度、今日/本週課表、近 8 週負荷圖、PB 表 |
| 全隊總覽 | 全隊 ACWR/準備度/傷患一覽，高風險與傷患警示條 |
...
版面底色是溫和的漸變（米白 → 淡霧綠 → 淡霧藍），重點色取自跑道內場的草綠
（`--accent #2f6b52`）。田徑場的紅膠面（`--track #bf4029`）只出現在 hero 的
跑道畫布裡，讓它成為畫面上唯一的高彩度物件。

首頁 `/` 的主視覺是一個 canvas 動畫（`static/js/track-hero.js`，純 vanilla JS、
無外部相依、遵守 `prefers-reduced-motion`）：一座**按規格繪製**的四百公尺標準田徑場
緩慢旋轉（wheel effect）。

幾何依 World Athletics 規格：直道 84.39 m × 2、內側緣石半徑 36.50 m、
第 1 道量距線 36.80 m、分道寬 1.22 m、共 8 道 —— 第 1 道一圈剛好 400.00 m。
起跑線不是畫上去的，是由 `locateBack()` 沿各道自身路線算出來的：

| 標示 | 畫法 |
| --- | --- |
| 100 m / 110 mH | 直道延伸段上的一條共用起跑線，八道齊平（無分道差） |
| 200 m / 400 m | 逐道由終點回量 200 / 400 m，分道差自然浮現（第 8 道 400 m 差 53.03 m） |
| 800 m | 一圈彎道的分道差 `π · (r₈ − r₁)`，break 後可切入內道 |
| 過程 mark | 第 1 道由終點回量的 100 / 200 / 300 m |
| 終點線 | 八道共用的白色粗線 |

場地顏色照真實場館：紅膠面、白色分道線與緣石、綠色內場（含足球場輪廓）。
標示文字會反向旋轉回正，輪子轉動時仍讀得到。兩種玩法：
...
速度維持），並逐一列出每 10 公尺分段時間。跑者在第 4 道上前進，
路線就是真實的一百公尺：由直道延伸段的起跑線一路衝到終點線。
...
```bash
python manage.py test          # 139 項，約 10 秒
```
