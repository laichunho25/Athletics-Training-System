...
### 3b. 驗 cron 自己那四把 R2 金鑰

Cron Job **沒有 Shell 分頁**，進不去打指令。所以 `purge_videos`
一開場就會打一次 `head_bucket` 並且印一行狀態——**這行就是唐一的驗證方法**，
而且它印在「沒有片可清」之前，所以不需要有東西可刪也驗得到。

Trigger Run 之後看 Logs 第一行：

| 看到 | 意思 |
|---|---|
| `R2：已連上 bucket「xxx」` | 四把金鑰都對✅ |
| `R2：沒有設定（四個 R2_* 環境變數要齊）` | 有變數漏了或者值是空的 |
| `R2：連不上 bucket「xxx」：...` | 變數有，但值錯了（複製貼上點錯、貼錯 bucket） |

連不上 R2 而又有片要刪的時候，指令會**一條都不刪**就失敗退出，
並且單條片的 R2 檔刪不掉時那一行紀錄也會留著。目的是不要製造孤兒：
資料庫那行沒了、R2 上的物件却留下來，那個檔就再也沒人指得到。

> 孤兒物件另有一個指令對帳：`python manage.py find_orphans`（加 `--apply` 才真刪）。
> 直傳是兩步的，上傳到一半斷掉就會在 R2 留下沒紀錄指著的檔；
> 這個指令不排 cron，幾個月自己跑一次、自己看過清單就好。
