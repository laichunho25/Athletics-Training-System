...
class User(AbstractUser):
    ...
    phone = models.CharField(_("電話"), max_length=30, blank=True)
    avatar = models.ImageField(_("頭像"), upload_to="avatars/", null=True, blank=True)
    #: 介面語言：教練／管理員／運動員各自選，換裝置換瀏覽器都跟著帳號走。
    language = models.CharField(
        _("介面語言"), max_length=10, choices=settings.LANGUAGES, default="zh-hant"
    )

    class Meta:
        # 後台 Accounts 區塊只保留四張表，這張是「管理員」（帳號與角色）
        verbose_name = _("管理員")
        verbose_name_plural = _("管理員")
...
class Event(models.Model):
    ...
    code = models.CharField(_("代碼"), max_length=20, unique=True)
    name_zh = models.CharField(_("中文名稱"), max_length=50)
    name_en = models.CharField(_("英文名稱"), max_length=50)
    category = models.CharField(_("類別"), max_length=15, choices=EventCategory.choices)
    unit = models.CharField(_("計量單位"), max_length=10, choices=MeasureUnit.choices)
    distance_m = models.PositiveIntegerField(_("距離 (m)"), null=True, blank=True)

    class Meta:
        verbose_name = _("田徑項目")
        verbose_name_plural = _("田徑項目")
...
class CoachProfile(TimeStampedModel):
    ...
    squad_name = models.CharField(_("組別名稱"), max_length=80, blank=True)
    specialties = models.CharField(_("專長"), max_length=120, blank=True)
    certification = models.CharField(_("執照"), max_length=120, blank=True)
    years_of_experience = models.PositiveSmallIntegerField(_("執教年資"), default=0)

    class Meta:
        verbose_name = _("教練檔案")
        verbose_name_plural = _("教練檔案")
...
class AthleteProfile(TimeStampedModel):
    ...
    primary_event = models.ForeignKey(
        Event, on_delete=models.PROTECT, related_name="primary_athletes", verbose_name=_("主項")
    )
    secondary_events = models.ManyToManyField(
        Event, blank=True, related_name="secondary_athletes", verbose_name=_("副項")
    )
    training_days_per_week = models.PositiveSmallIntegerField(_("每週訓練日數"), default=5)
    strength_experience_years = models.DecimalField(
        _("力量訓練年資"), max_digits=3, decimal_places=1, default=0
    )
    status = models.CharField(
        _("目前狀態"), max_length=10, choices=AthleteStatus.choices, default=AthleteStatus.HEALTHY
    )
...
class PersonalBest(TimeStampedModel):
    ...
    mark = models.DecimalField(_("成績"), max_digits=8, decimal_places=2, help_text=_("時間存秒，距離存公尺"))
    wind = models.DecimalField(_("風速 (m/s)"), max_digits=4, decimal_places=1, null=True, blank=True)
    date = models.DateField(_("創造日期"))
    competition_name = models.CharField(_("賽事名稱"), max_length=120, blank=True)
    is_current = models.BooleanField(_("目前最佳"), default=True)
...
class BodyMetricLog(TimeStampedModel):
    ...
    class Source(models.TextChoices):
        MANUAL = "MANUAL", _("手動輸入")
        IMPORT = "IMPORT", _("檔案匯入")
    ...
    body_fat_pct = models.DecimalField(
        _("體脂肪率 (%)"), max_digits=4, decimal_places=1, null=True, blank=True
    )
    muscle_mass_kg = models.DecimalField(
        _("肌肉量 (kg)"), max_digits=5, decimal_places=2, null=True, blank=True
    )
    muscle_mass_index = models.SmallIntegerField(_("肌肉量判定指數"), null=True, blank=True)
    bmi = models.DecimalField("BMI", max_digits=4, decimal_places=1, null=True, blank=True)
    muscle_quality_score = models.SmallIntegerField(_("肌肉質量指數"), null=True, blank=True)
    visceral_fat_level = models.DecimalField(
        _("內臟脂肪等級"), max_digits=4, decimal_places=1, null=True, blank=True
    )
    bone_mass_kg = models.DecimalField(
        _("推定骨量 (kg)"), max_digits=4, decimal_places=2, null=True, blank=True
    )
    body_water_pct = models.DecimalField(
        _("體水分率 (%)"), max_digits=4, decimal_places=1, null=True, blank=True
    )
    ...
    muscle_arm_r = models.DecimalField(
        _("右上肢肌肉量 (kg)"), max_digits=5, decimal_places=2, null=True, blank=True
    )
    muscle_arm_l = models.DecimalField(
        _("左上肢肌肉量 (kg)"), max_digits=5, decimal_places=2, null=True, blank=True
    )
    muscle_leg_r = models.DecimalField(
        _("右腳肌肉量 (kg)"), max_digits=5, decimal_places=2, null=True, blank=True
    )
    muscle_leg_l = models.DecimalField(
        _("左腳肌肉量 (kg)"), max_digits=5, decimal_places=2, null=True, blank=True
    )
    muscle_trunk = models.DecimalField(
        _("軀幹部位肌肉量 (kg)"), max_digits=5, decimal_places=2, null=True, blank=True
    )

    # -------------------------------------------------------- 部位脂肪率
    fat_arm_r = models.DecimalField(
        _("右上肢脂肪率 (%)"), max_digits=4, decimal_places=1, null=True, blank=True
    )
    fat_arm_l = models.DecimalField(
        _("左上肢脂肪率 (%)"), max_digits=4, decimal_places=1, null=True, blank=True
    )
    fat_leg_r = models.DecimalField(
        _("右腳脂肪率 (%)"), max_digits=4, decimal_places=1, null=True, blank=True
    )
    fat_leg_l = models.DecimalField(
        _("左腳脂肪率 (%)"), max_digits=4, decimal_places=1, null=True, blank=True
    )
    fat_trunk = models.DecimalField(
        _("軀幹部位脂肪率 (%)"), max_digits=4, decimal_places=1, null=True, blank=True
    )

    # ---------------------------------------------------- 部位肌肉品質點數
    mq_arm_r = models.SmallIntegerField(_("右上肢肌肉品質點數"), null=True, blank=True)
    mq_arm_l = models.SmallIntegerField(_("左上肢肌肉品質點數"), null=True, blank=True)
    mq_leg_r = models.SmallIntegerField(_("右腳肌肉品質點數"), null=True, blank=True)
    mq_leg_l = models.SmallIntegerField(_("左腳肌肉品質點數"), null=True, blank=True)
    mba_rating = models.CharField(_("MBA 判定"), max_length=20, blank=True)

    # -------------------------------------------------------------- 其他
    resting_hr = models.PositiveSmallIntegerField(_("靜息心率"), null=True, blank=True)
    hrv = models.PositiveSmallIntegerField("HRV (ms)", null=True, blank=True)
    note = models.TextField(_("備註"), blank=True)
    source = models.CharField(
        _("來源"), max_length=10, choices=Source.choices, default=Source.MANUAL
    )
    source_file = models.CharField(_("匯入檔名"), max_length=120, blank=True)
